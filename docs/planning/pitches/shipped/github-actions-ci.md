...
Shipped the full default path, no cuts. `.github/workflows/ci.yml` has four jobs: `changes` (paths-filter), `web-ci` (lint + typecheck + build, ~60s budget), `web-smoke` (Playwright against the Vercel preview URL, ~120s budget, PR-only), and stub `api-ci`/`ios-ci` that wake up when their dirs land. Added `concurrency: cancel-in-progress` for non-main branches so rapid pushes don't pile up runs. Playwright config defaults `baseURL` to `https://www.runtime.training` when `PREVIEW_URL` isn't set — that single design choice means `npm run test:smoke` works locally with no env setup, which made the wire-up verifiable before any PR ran.

Smoke against production passed in 2.3s on first attempt — wordmark renders with the `role="img"` + `aria-label="runtime"` accessibility hooks the [[shipped/wordmark-as-component]] pitch put in place. Good moment to recognize that decision earning its keep: the smoke assertion is one line because the wordmark already announces itself correctly to assistive tech.

What's deferred (intentional): `migrations.yml` (no Supabase project yet), richer Playwright coverage (one smoke page is all there is to test), and the branch protection rule on `main` — that's a one-click GitHub UI step *after* CI's first successful run reports a result, so it lands as a manual follow-up rather than in this pitch.

Worth watching:
- The first PR is the real verification. If `dorny/paths-filter` or `patrickedqvist/wait-for-vercel-preview` doesn't behave, patch from PR logs.
- macOS runners at 10× ubuntu cost mean the `ios-ci` stub is free today but consequential when block 5 lands. Possibly run SwiftLint on ubuntu then.
- `packages/**` triggers `web-ci`, including docs-only changes to `packages/design-tokens/README.md`. Over-trigger on the cautious side; cost is one extra build per docs PR.
