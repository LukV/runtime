...
5. **Wordmark-as-SVG.** Hot-take worth flagging: since the wordmark is the only place Source Serif appears, we could export it as an SVG path and ship the app *without bundling Source Serif at all*. Saves ~80kb in the iOS bundle, makes the mark renderable at any size without font dependency, and the amber period becomes a separate `<circle>` element that's trivially themable. Worth a 30-minute experiment.
   ...
   **Resolved 2026-05-27 in [[font-integration]] and [[wordmark-as-component]]:** *yes, inline SVG on both platforms.* The wordmark is generated once from Source Serif 4 Medium glyph paths, committed to `packages/design-tokens/dist/wordmark.svg`, and rendered identically on web and iOS — a single source of truth. The amber period is a separate `<circle>` element with `fill="var(--color-eerste-licht)"` so the brand device never depends on font loading. Source Serif 4 is loaded on the web (for the cv11/tnum-flavoured UI work and other surfaces that might use it later) but the wordmark component itself doesn't reach for it. Source Serif 4 is **not** bundled in the iOS app at all.
